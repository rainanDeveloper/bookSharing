	...
	async login(request, response){
		...
		if(typeof(usr_login)=="string"&&typeof(usr_pass)=="string"){
			...
			if(User){
				...
				var hash = crypto.createHash('sha256').update(usr_pass+User.usr_salt).digest('hex')
				hash = crypto.createHash('sha256').update(hash+User.usr_salt).digest('hex')
				hash = crypto.createHash('sha256').update(hash+User.usr_salt).digest('hex')

				console.log(User)

				if(User.usr_pass===hash){
					const id = User.id
					...
					response.json({auth: true, token: token, username: User.usr_name})
				}
				else{
					return response.status(401).json({success: false, messageError: "Incorrect password"})
				}
			}
			else{
				return response.status(401).json({success: false, messageError: "User not found"})
			}
		}
	},
